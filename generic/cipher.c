...
//>>>
static int decrypt_chunk(Tcl_Interp* interp, struct cipher_md* md, Tcl_Obj* ciphertext, Tcl_Obj** plaintext) //<<<
{
	int						code = TCL_OK;
	Tcl_Obj*				res = NULL;
	const unsigned char*	in = NULL;
	...
	int err;
	switch (md->mode) {
#define X(lower, upper) case CM_##upper: err = lower##_decrypt(in, out, inlen, &md->state_##lower); break;
		CIPHER_MODES_MAP
#undef X
		default: THROW_PRINTF_LABEL(finally, code, "Unhandled cipher mode %d", md->mode);
	}
	...
	if (err != CRYPT_OK) {
		Tcl_SetErrorCode(interp, "TOMCRYPT", "CIPHER", "DECRYPT", cipher_mode_strs[md->mode], NULL);
		THROW_PRINTF_LABEL(finally, code, "decryption failed: %s", error_to_string(err));
	}

	replace_tclobj(plaintext, res);
...
//>>>
static int decrypt_final(Tcl_Interp* interp, struct cipher_md* md, Tcl_Obj** plaintext) //<<<
{
	int				code = TCL_OK;
	Tcl_Obj*		tmp = NULL;
	cipher_spec*	spec = NULL;
		...
		case CM_CBC:	// Unpad
			{
				...
				if (len > 0) { // len should never be zero, but protect against underflow
					...
					len--;
					while (bytes[len-1] == last) len--;
					Tcl_SetByteArrayLength(*plaintext, len);
				}
			}
			break;
...
OBJCMD(cipher_decrypt_cmd) //<<<
{
	...
	// Encrypt
	TEST_OK_LABEL(finally, code, decrypt_chunk(interp, &md, objv[A_DATA], &res));
	TEST_OK_LABEL(finally, code, decrypt_final(interp, &md, &res));
